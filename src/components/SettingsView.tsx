...
import { 
  User, Building2, BellRing, Save, Database, Plus, Users, Shield, Trash2, Mail
} from "lucide-react";
// Importamos tanto el cliente normal como el Admin
...
export default function SettingsView() {
  ...
  const [newEstablecimiento, setNewEstablecimiento] = useState({ 
    ...
    adminNombre: '', adminCorreo: '', adminPassword: '',
    enviarCorreo: true
  });
  
  const [newPersonal, setNewPersonal] = useState({ nombre: '', correo: '', rol: 'Medico', password: '' });
  ...
  const cargarDatosConfiguracion = async () => {
    try {
      ...
      if (profile?.rol === 'SuperAdmin') {
        const { data: estData } = await supabase.from('establecimientos').select('*').order('fecha_registro', { ascending: false });
  ...
  // 1. LÓGICA SUPERADMIN: Crear Establecimiento y su Administrador
  const handleCrearEstablecimiento = async () => {
    ...
    try {
      ...
      if (estCreado) {
        // CREAMOS EL USUARIO EN EL SISTEMA DE LOGIN REAL (Auth)
        ...
        // Guardamos el perfil vinculándolo con el ID real de Auth
        if (authData.user) {
        ...
        if (newEstablecimiento.enviarCorreo) {
          console.log(`Credenciales creadas y listas para: ${newEstablecimiento.adminCorreo}`);
        }
      }
  ...
  // 2. LÓGICA ADMIN ESTABLECIMIENTO: Crear Personal Médico
  const handleCrearPersonal = async () => {
    ...
    try {
      ...
      if (authData.user) {
        const { error: errUser } = await supabase.from('usuarios').insert([{
          id_usuario: authData.user.id,
          id_establecimiento: userProfile.id_establecimiento, // Hereda el establecimiento del Admin
          ...
          <section className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input type="text" defaultValue={userProfile?.nombre_completo} className="w-full px-4 py-2 border rounded-lg text-sm bg-slate-50 outline-none" disabled />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 uppercase">Correo Electrónico</label>
                <input type="email" defaultValue={userProfile?.correo_electronico} className="w-full px-4 py-2 border rounded-lg text-sm bg-slate-50 outline-none" disabled />
              </div>
            </div>
          </section>
          ...
          {isSuperAdmin && (
            <section className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
                  ...
                  <div className="p-5 space-y-6">
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <input type="text" placeholder="Nombre (Ej: Clínica Los Ángeles)" value={newEstablecimiento.nombre} onChange={e => setNewEstablecimiento({...newEstablecimiento, nombre: e.target.value})} className="px-3 py-2 text-sm border rounded-lg outline-none focus:border-emerald-400" />
                        <input type="text" placeholder="Ciudad / Municipio" value={newEstablecimiento.ciudad} onChange={e => setNewEstablecimiento({...newEstablecimiento, ciudad: e.target.value})} className="px-3 py-2 text-sm border rounded-lg outline-none focus:border-emerald-400" />
                        <select value={newEstablecimiento.tipo} onChange={e => setNewEstablecimiento({...newEstablecimiento, tipo: e.target.value})} className="px-3 py-2 text-sm border rounded-lg outline-none bg-white focus:border-emerald-400">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                        <input type="text" placeholder="Nombre del encargado" value={newEstablecimiento.adminNombre} onChange={e => setNewEstablecimiento({...newEstablecimiento, adminNombre: e.target.value})} className="px-3 py-2 text-sm border rounded-lg outline-none focus:border-emerald-400" />
                        <input type="email" placeholder="Correo administrativo" value={newEstablecimiento.adminCorreo} onChange={e => setNewEstablecimiento({...newEstablecimiento, adminCorreo: e.target.value})} className="px-3 py-2 text-sm border rounded-lg outline-none focus:border-emerald-400" />
                        <input type="text" placeholder="Contraseña temporal" value={newEstablecimiento.adminPassword} onChange={e => setNewEstablecimiento({...newEstablecimiento, adminPassword: e.target.value})} className="px-3 py-2 text-sm border rounded-lg outline-none focus:border-emerald-400" />
                      </div>
                      
                      <label className="flex items-center gap-2 cursor-pointer mt-2 w-fit">
                        <input 
                          type="checkbox" 
                          checked={newEstablecimiento.enviarCorreo}
                          onChange={e => setNewEstablecimiento({...newEstablecimiento, enviarCorreo: e.target.checked})}
                          className="w-4 h-4 text-emerald-600 rounded border-slate-300 focus:ring-emerald-500"
                        />
                        <span className="text-sm font-medium text-slate-600 flex items-center gap-1">
                          <Mail className="w-4 h-4 text-slate-400" />
                          Enviar credenciales y enlace de acceso por correo
                        </span>
                      </label>
                    </div>

                    <div className="flex justify-end gap-2 pt-2">
                      <button onClick={() => setShowForm(false)} className="px-5 py-2 text-sm font-bold text-slate-500 hover:bg-slate-200 rounded-lg transition">Cancelar</button>
                      <button onClick={handleCrearEstablecimiento} className="px-5 py-2 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg shadow-sm transition">Crear Centro y Admin</button>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="overflow-x-auto">
                  ...
                  <tbody>
                    {establecimientos.map(est => (
                      <tr key={est.id_establecimiento} className="border-b hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3"><span className={`px-2 py-1 text-[10px] font-bold rounded-full ${est.tipo === 'Farmacia' ? 'bg-blue-100 text-blue-700' : 'bg-emerald-100 text-emerald-700'}`}>{est.tipo}</span></td>
                ...
                {establecimientos.length === 0 && <p className="text-center text-sm text-slate-400 py-4">No hay establecimientos registrados.</p>}
              </div>
            </section>
          )}
          ...
          {isAdminEstablecimiento && (
            <section className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
              ...
              {showForm && (
                <div className="mb-6 p-4 border border-blue-100 bg-blue-50 rounded-lg space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <input type="text" placeholder="Nombre completo" value={newPersonal.nombre} onChange={e => setNewPersonal({...newPersonal, nombre: e.target.value})} className="px-3 py-2 text-sm border rounded outline-none" />
                    <input type="email" placeholder="Correo electrónico" value={newPersonal.correo} onChange={e => setNewPersonal({...newPersonal, correo: e.target.value})} className="px-3 py-2 text-sm border rounded outline-none" />
                    <select value={newPersonal.rol} onChange={e => setNewPersonal({...newPersonal, rol: e.target.value})} className="px-3 py-2 text-sm border rounded outline-none bg-white">
                      <option value="Medico">Médico</option>
                      <option value="Enfermero">Enfermero/a</option>
                      <option value="Farmaceutico">Farmacéutico/a</option>
                    </select>
                    <input type="text" placeholder="Contraseña temporal" value={newPersonal.password} onChange={e => setNewPersonal({...newPersonal, password: e.target.value})} className="px-3 py-2 text-sm border rounded outline-none" />
                  </div>
                  <div className="flex justify-end gap-2">
                    <button onClick={() => setShowForm(false)} className="px-4 py-2 text-xs font-bold text-slate-500 hover:bg-slate-200 rounded">Cancelar</button>
                    <button onClick={handleCrearPersonal} className="px-4 py-2 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 rounded shadow-sm">Crear Cuenta</button>
                  </div>
                </div>
              )}
              ...
              <div className="overflow-x-auto">
                  ...
                  <tbody>
                    {personal.map(p => (
                      <tr key={p.id_usuario} className="border-b hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3"><span className="px-2 py-1 text-[10px] font-bold rounded-full bg-slate-100 text-slate-600">{p.rol}</span></td>
                        <td className="px-4 py-3 text-right">
                          <button className="text-red-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
                        </td>
                      </tr>
                ...
                {personal.length === 0 && <p className="text-center text-sm text-slate-400 py-4">Aún no hay personal registrado en este centro.</p>}
              </div>
            </section>
          )}
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 bg-slate-800 text-white rounded-xl shadow-sm overflow-hidden relative group">
            ...
            <div className="flex items-center gap-2 mt-2 text-emerald-400">
              ...
              <span className="text-xs font-medium">Sincronizado con BD</span>
            </div>
          </div>
        </div>
      </div>
